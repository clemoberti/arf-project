...
class TestImageProcessing(unittest.TestCase):
    ...
    def test_get_dictonary_without_noise(self):
        ...
        h = 32
        step = 32
        all_patches_n = ip.get_all_patches(image, h,step).shape[0]
        
        incomplete_patches_n = ip.get_incomplete_patches(image,h,step).shape[0]
        self.assertEqual(incomplete_patches_n,0)
        
        self.assertEqual(ip.get_dictionnary_patches(image, h,step).shape[0], all_patches_n)
    ...
    def test_get_dictonary_with_one_black_box(self):
        ...
        incomplete_patches_n = ip.get_incomplete_patches(modified_image,h, step).shape[0]
        self.assertNotEqual(incomplete_patches_n,0)
        
        dictionnary_n = ip.get_dictionnary_patches(modified_image, h, step).shape[0]
